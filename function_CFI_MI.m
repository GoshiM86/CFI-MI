...
% modified mutual information applied to binary profiles of neural spiking activity 
% (states of neural quiescence '0' and states of firing '1' unfolded in time) 
...
for i = 1 : numel(spike_trains_labels) % two spike trains, pairwise measure
        ...
        if ~isempty(xx)
            for f = 1 : numel(d1)
                ...
                if upper_limit > numel(spike_train)
                    upper_limit = xx(d1(f));
                    spike_ranges = [spike_ranges; spike_train(down_limit) spike_train(upper_limit)]; % all intervals under the temporary state ('1' or '0')
                else
                    spike_ranges = [spike_ranges; spike_train(down_limit) spike_train(upper_limit)]; % all intervals under the temporary state ('1' or '0')
                end
            end
...
% marginal_idle1 = marginal_prob_train1(1,:); % no needed!
